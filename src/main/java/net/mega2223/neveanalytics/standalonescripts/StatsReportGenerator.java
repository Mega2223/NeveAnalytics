...
public class StatsReportGenerator {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            System.out.println("SNOW PIXELS FOR "+ files[i].getName() +": " + snowPixels + " (Threshold "+SNOW_THRESHOLD+")");
            float x = geoTransform.get(1).getAsFloat(), y = geoTransform.get(5).getAsFloat();
            x = Math.abs(x); y = Math.abs(y);
            System.out.println("PIXEL SIZE: (" + x + "," + y + ")");
            System.out.println("TOTAL AREA COVERED: " + snowPixels * Math.sqrt(x * y) + ", " + 100*((float)snowPixels)/(raster.getNumPixels()) + "%");
            System.out.println("------------------------------------------------------------");
        }
